...
function PracticePanel({ module, symbol, source, limit, bars, contextInputs = {}, targetConcept }: {
  ...
  const run = async () => {
    ...
    if (concept.input_kind === 'market_bars' && !bars?.length && concept.id !== 'book_pitfall_repainting') {
    ...
    setError('');
    try {
      setResult(await api.runPractice({ concept_id: concept.id, module, symbol, source, limit, inputs: parsed, bars: concept.id === 'book_pitfall_repainting' ? undefined : bars?.length ? bars : undefined }));
    } catch (e) {
      setError(String(e));
      setResult(null);
