...
const SERVER_FETCHED_PRACTICES = new Set(['book_pitfall_repainting', 'book_pitfall_timeframe', 'book_pitfall_formula_variant', 'book_pitfall_open_candle', 'book_trade_volume', ...FLOW_PRACTICES]);
...
function PracticePanel({ module, symbol, source, limit, bars, contextInputs = {}, targetConcept }: {
  ...
  const run = async () => {
    ...
    setError('');
    try {
      setResult(await api.runPractice({ concept_id: concept.id, module, symbol, source, limit, inputs: parsed, bars: SERVER_FETCHED_PRACTICES.has(concept.id) ? undefined : bars?.length ? bars : undefined }));
    } catch (e) {
      setError(String(e));
      setResult(null);
