...
const RECENT_TRADE_PRACTICES = new Set(['book_net_volume', 'volume_profile']);
const BLOCK_PRACTICES = new Set(['book_block_height', 'book_block_size']);
const SERVER_FETCHED_PRACTICES = new Set(['book_pitfall_repainting', 'book_pitfall_timeframe', 'book_pitfall_formula_variant', 'book_pitfall_open_candle', 'book_trade_volume', 'book_52w_range', ...FLOW_PRACTICES, ...DEPTH_PRACTICES, ...PAIR_PRACTICES, ...RECENT_TRADE_PRACTICES, ...BLOCK_PRACTICES]);
...
const CHINESE_FIELDS: Record<string, string> = {
  ...
  book_nonstandard_bar: 'OHLC4 合成展示价', actual_close: '真实收盘价', synthetic_minus_close: '合成价与真实收盘价之差', book_period: '来源约定的名义周期', last_observed_interval_seconds: '最近相邻观测间隔', bar_count: 'K线数', missing_expected_intervals: '缺失的预期小时数', calendar_gap_count: '股票日期空档数',
  source_bar_seconds: '来源 K 线周期', same_completed_asof: '短长窗口使用同一已收盘截止点', short_horizon_bars: '短期窗口 K 线数', long_horizon_bars: '长期窗口 K 线数', short_horizon_return: '短期窗口收盘价变化', long_horizon_return: '长期窗口收盘价变化', horizon_direction_differs: '短长窗口方向不同', close_price: '收盘价', short_horizon_start: '短期窗口起点（5 根）', long_horizon_start: '长期窗口起点（20 根）', macd_histogram_x1: 'MACD 柱体（x1 约定）', macd_histogram_x2: 'MACD 柱体（x2 约定）', latest_histogram_difference: '最新两种柱体约定之差', latest_histogram_x1: '最新 MACD 柱体（x1）', latest_histogram_x2: '最新 MACD 柱体（x2）', fast_period: '快线 EMA 周期', slow_period: '慢线 EMA 周期', signal_period: '信号 EMA 周期', first_histogram_scale: '第一种柱体缩放', second_histogram_scale: '第二种柱体缩放',
  ...
  base_volume: '标的成交数量', quote_volume: '实际成交额', vwap: '最新一小时成交均价',
...
function practiceUnit(unit: string | undefined, result: PracticeResult) {
  if (unit === 'base_asset') return result.asset_units?.base_asset || '标的基本单位';
  if (unit === 'quote_asset') return result.asset_units?.quote_asset || '计价资产';
  if (unit === 'quote_asset_per_base') return `${result.asset_units?.quote_asset || '计价资产'}/${result.asset_units?.base_asset || '标的基本单位'}`;
  return chineseUnit(unit);
}
...
function CandlePatternVisual({ concept }: { concept: KnowledgeEntry }) {
  ...
  useEffect(() => {
    ...
    practiceCatalog().then(async catalog => {
      ...
      const value = await api.runPractice({ concept_id: concept.id, module: 'data', symbol: 'BTCUSDT', second_symbol: PAIR_PRACTICES.has(concept.id) ? 'ETHUSDT' : undefined, source: 'binance', limit: BLOCK_PRACTICES.has(concept.id) ? 10 : 80, inputs: {} });
...
function PracticePanel({ module, symbol, source, limit, bars, contextInputs = {}, targetConcept }: {
  ...
  const run = async () => {
    ...
    if (concept.input_kind === 'market_bars' && !bars?.length && !SERVER_FETCHED_PRACTICES.has(concept.id)) {
    ...
    setError('');
    try {
      setResult(await api.runPractice({ concept_id: concept.id, module, symbol, second_symbol: PAIR_PRACTICES.has(concept.id) ? secondSymbol : undefined, source, limit: BLOCK_PRACTICES.has(concept.id) ? 10 : DEPTH_PRACTICES.has(concept.id) ? 5 : limit, inputs: parsed, bars: SERVER_FETCHED_PRACTICES.has(concept.id) ? undefined : bars?.length ? bars : undefined }));
    } catch (e) {
      setError(String(e));
      setResult(null);
  ...
  return (
    <section className="ax-section">
      ...
      <div className="ax-controls">
        ...
        <label>数据源
          <Dropdown label="数据源" options={MARKET_SOURCE_OPTIONS}
            value={source} onChange={(v: SourceType) => { if (v === source) return; setSource(v); setSymbol(MARKET_DEFAULT_SYMBOL[v]); setResult(null); }} minWidth={140} />
        </label>
        <label>K 线数
  ...
  return (
    <section className="ax-section">
      ...
      <div className="ax-controls">
        ...
        <label>数据源
          <Dropdown label="数据源" options={MARKET_SOURCE_OPTIONS}
            value={source} onChange={(v: SourceType) => { if (v === source) return; setSource(v); setSymbol(MARKET_DEFAULT_SYMBOL[v]); setResults([]); }} minWidth={140} />
        </label>
        <label>资金
          <input type="number" value={capital} onChange={e => setCapital(parseFloat(e.target.value) || 10000)} step={1000} />
        </label>
