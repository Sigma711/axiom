...
const SERVER_FETCHED_PRACTICES = new Set(['book_pitfall_repainting', 'book_pitfall_timeframe', 'book_pitfall_formula_variant', 'book_pitfall_open_candle', 'book_trade_volume', 'book_52w_range', ...FLOW_PRACTICES, ...DEPTH_PRACTICES, ...PAIR_PRACTICES, ...RECENT_TRADE_PRACTICES]);
...
function CandlePatternVisual({ concept }: { concept: KnowledgeEntry }) {
  ...
  return <Section label="可计算示例" highlight>
    <p className="ax-practice-note">{result.recent_trades ? '基于 Binance 最近逐笔成交计算；窗口时长随成交速度变化，不代表全天。' : result.pair ? `基于 ${result.pair.first_symbol} 与 ${result.pair.second_symbol} 同时刻的真实已收盘小时线计算；不补缺，也不把样本内关系当作交易建议。` : result.provenance === 'provided_market_bars' ? '基于已收盘的真实市场行情计算，用来观察数值变化，不代表交易建议。' : '基于可编辑教学输入计算，不代表当前币种行情。'}</p>
    {isCandlePattern ? <CandlePatternVisual concept={concept} /> : (result.recent_trades ? <RecentTradesVisual result={result} /> : result.chart ? <BookChartVisual chart={result.chart} name={concept.name} /> : series && values.length > 1 ? <KnowledgeSeriesVisual name={concept.name} result={result} /> : <ScalarKnowledgeDiagram concept={concept} inputs={inputs} scalar={scalar} unit={scalar ? result.units?.[scalar[0]] : undefined} />)}
    {!result.recent_trades && <p className="ax-practice-reading">{resultSentence(concept.name, result.values, result.units, Boolean(series && values.length > 1))}</p>}
    {!result.recent_trades && result.notes.slice(0, 1).map(note => <p className="ax-practice-note" key={note}>{note}</p>)}
  </Section>;
}
