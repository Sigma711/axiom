...
const labels: Record<string, string> = { upper: '上轨', middle: '中轨', lower: '下轨', macd: 'MACD 快慢线差', signal: '信号线', histogram: '柱状差值', close: '收盘价', price: '价格', drawdown: '回撤', equity: '净值', volume: '成交量', hourly_volume: '每小时成交量', plus_di: '正向指标', minus_di: '负向指标', relative_strength_line: '同时间收盘价之比', spread: '配对价差', zscore: '价差 Z 分数', in_sample_ols_residual: '样本内 OLS 残差', pivot_high_occurrence: '局部高点发生位置（仅回看）', pivot_low_occurrence: '局部低点发生位置（仅回看）', confirmed_pivot_high: '局部高点确认价（t+2）', confirmed_pivot_low: '局部低点确认价（t+2）', confirmation_delay_bars: '确认延迟', close_price: '真实收盘价', short_horizon_start: '短期窗口起点（5 根）', long_horizon_start: '长期窗口起点（20 根）', macd_histogram_x1: 'MACD 柱体 ×1', macd_histogram_x2: 'MACD 柱体 ×2', histogram_scale_difference: '两种口径差值', last_completed_close_series: '上一根已收盘价', current_candle_open_series: '当前 K 线开盘价', provisional_close_series: '当前临时价（尚未收盘）', base_volume_series: '标的成交数量', quote_volume_series: '实际成交额' };
...
const number = (v: number) => Math.abs(v) >= 1e8 ? `${+(v / 1e8).toPrecision(3)}亿` : Math.abs(v) >= 10000 ? `${+(v / 10000).toPrecision(4)}万` : `${+v.toPrecision(4)}`;
...
export function KnowledgeSeriesVisual({ name, result }: { name: string; result: PracticeResult }) {
  ...
  const tradeVolume = result.concept_id === 'book_trade_volume';
  const wide = repainting || openCandle || tradeVolume || Boolean(result.pair) || result.series.some(series => series.name === 'short_horizon_start' || series.name === 'macd_histogram_x1');
  const right = wide ? 736 : 396;
  const left = tradeVolume ? 90 : 64;
  return <figure className={`ax-knowledge-chart ax-series-illustration${wide ? ' ax-wide-series-chart' : ''}${repainting ? ' ax-repainting-chart' : ''}${tradeVolume ? ' ax-trade-volume-chart' : ''}`}>
    <svg viewBox={`0 0 ${wide ? 760 : 420} ${panels.length * 185}`} role="img" aria-label={`${name} 全部序列，按单位分图`}>
      {panels.map(([unit, series], panel) => {
        ...
        const min = tradeVolume ? 0 : lo - padding, max = hi + padding;
        ...
        return <g key={unit} transform={`translate(0,${panel * 185})`}>
          <text x={left} y="20">{unit === 'base_asset' ? `成交数量（${result.asset_units?.base_asset || '标的基本单位'}）` : unit === 'quote_asset' ? `实际成交额（${result.asset_units?.quote_asset || '计价资产'}）` : units[unit] || unit}</text>
          {[min, (min + max) / 2, max].map(value => <g key={value}><line x1={left} x2={right} y1={y(value)} y2={y(value)} /><text x={left - 8} y={y(value) + 4} textAnchor="end">{number(value)}</text></g>)}
          <text x={left} y="171">{sampleLabel(0)}</text><text x={right} y="171" textAnchor="end">{sampleLabel(count - 1)}</text>
          {series.map((s, trace) => {
            ...
            const color = tradeVolume && unit === 'quote_asset' ? colors[1] : colors[trace % colors.length];
            return <g key={s.name} aria-label={labels[s.name] || s.name}>
              {tradeVolume ? s.values.map((value, i) => !finite(value) ? null : <rect key={i} data-volume-bar={s.name} x={x(i) - Math.max(2, (right - left) / count * 0.32)} y={y(value)} width={Math.max(4, (right - left) / count * 0.64)} height={Math.max(0, y(0) - y(value))} fill={color}><title>{`${labels[s.name] || s.name} · ${sampleLabel(i)} · ${number(value)}`}</title></rect>) : <>
    ...
    <figcaption><div className="ax-series-legend">{panels.flatMap(([unit, series]) => series.map((s, i) => <span key={`${unit}-${s.name}`}><i className={s.name.endsWith('_occurrence') ? 'ax-legend-retrospective' : s.name.startsWith('confirmed_pivot') ? 'ax-legend-confirmed' : ''} style={{ backgroundColor: tradeVolume && unit === 'quote_asset' ? colors[1] : colors[i % colors.length], color: tradeVolume && unit === 'quote_asset' ? colors[1] : colors[i % colors.length] }} />{labels[s.name] || s.name}</span>))}</div>{openCandle ? '横轴为 Binance 实际 K 线开盘时间；右侧仅为抓取时的临时快照，尚未收盘，数值可能继续变化。' : tradeVolume ? '横轴为已收盘 Binance K 线时间；成交额来自交易所计价资产成交量字段，不用收盘价乘成交数量估算。窄屏可左右滑动查看完整24根。' : result.pair ? `横轴为 ${result.pair.first_symbol} 与 ${result.pair.second_symbol} 同时刻的已收盘小时线；两只标的缺失时不填补。不同单位分图，空白保留预热期或无定义结果。` : <>{wide && result.bars?.length ? '横轴按实际 K 线时间排序；' : '横轴为样本顺序；'}不同单位分图，空白保留预热期或缺失数据。</>}</figcaption>
  </figure>;
}
