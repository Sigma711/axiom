...
impl RsiStrategy {
    ...
    fn calc_rsi(&self) -> Option<f64> {
        ...
        ag /= self.period as f64;
        al /= self.period as f64;
        if al == 0.0 {
            return Some(100.0);
        }
...
impl Strategy for RandomStrategy {
    ...
    fn on_bar(&mut self, bar: &Bar) -> Signal {
        ...
        if r < self.buy_prob {
            make_signal(bar, Side::Buy, 0.5, "随机买入".into())
        } else if r > 1.0 - self.sell_prob {
            make_signal(bar, Side::Sell, 0.5, "随机卖出".into())
        } else {
            hold(bar, "随机观望")
        }
    }
        ...
        if let Some(p) = prev {
            if p <= 0.0 && diff > 0.0 {
                return make_signal(
                    ...
                    Side::Buy,
                    0.8,
                    format!("MACD 金叉 DIF={:.4} DEA={:.4}", d, dea_v),
                );
            }
            if p >= 0.0 && diff < 0.0 {
                return make_signal(
                    ...
                    Side::Sell,
                    0.8,
                    format!("MACD 死叉 DIF={:.4} DEA={:.4}", d, dea_v),
                );
            }
        }
    ...
    fn on_bar(&mut self, bar: &Bar) -> Signal {
        ...
        if bar.close <= lower {
            return make_signal(
                ...
                Side::Buy,
                0.8,
                format!("触下轨 c={:.2} lower={:.2}", bar.close, lower),
            );
        }
        if bar.close >= upper {
            return make_signal(
                ...
                Side::Sell,
                0.8,
                format!("触上轨 c={:.2} upper={:.2}", bar.close, upper),
            );
        }
...
impl Strategy for SupertrendStrategy {
    ...
    fn on_bar(&mut self, bar: &Bar) -> Signal {
        self.history.push(bar);
        if self.history.bars.len() < self.period + 1 {
            return hold(bar, "等待预热");
        }
        let st =
            crate::indicators::trend::supertrend(&self.history.bars, self.period, self.multiplier);
        let n = self.history.bars.len();
        let dir = match st.direction[n - 1] {
            Some(d) => d,
            None => return hold(bar, "ST 未就绪"),
        };
        let trend = st.trend[n - 1].unwrap();
        let prev = self.last_dir;
        self.last_dir = dir;
        if prev == -1 && dir == 1 {
            return make_signal(
                bar,
                Side::Buy,
                1.0,
                format!("Supertrend 翻多 线={:.2}", trend),
            );
        }
        if prev == 1 && dir == -1 {
            return make_signal(
                ...
                Side::Sell,
                1.0,
                format!("Supertrend 翻空 线={:.2}", trend),
            );
        }
...
impl Strategy for DonchianBreakoutStrategy {
    ...
    fn on_bar(&mut self, bar: &Bar) -> Signal {
        self.history.push(bar);
        if self.entry_period == 0 || self.exit_period == 0 {
            return hold(bar, "通道周期必须大于 0");
        }
        let n = self.history.bars.len();
        if n <= self.entry_period.max(self.exit_period) {
            return hold(bar, "等待预热");
        }
        let entry_window = &self.history.bars[n - 1 - self.entry_period..n - 1];
        let exit_window = &self.history.bars[n - 1 - self.exit_period..n - 1];
        let upper = entry_window
            .iter()
            .map(|b| b.high)
            .fold(f64::NEG_INFINITY, f64::max);
        let lower = exit_window
            .iter()
            .map(|b| b.low)
...
impl Strategy for VwapReversionStrategy {
    ...
    fn on_bar(&mut self, bar: &Bar) -> Signal {
        self.history.push(bar);
        if self.period == 0 {
            return hold(bar, "VWAP 周期必须大于 0");
        }
        if self.history.bars.len() < self.period {
            return hold(bar, "等待预热");
        }
        let n = self.history.bars.len();
        let window = &self.history.bars[n - self.period..];
        let volume = window.iter().map(|b| b.volume).sum::<f64>();
        if volume <= 0.0 {
            return hold(bar, "VWAP 成交量为 0");
        }
        let vwap_v = window
            .iter()
        ...
        if let Some(p) = prev {
            if p < 0.0 && j >= 0.0 {
                return make_signal(
                    ...
                    Side::Buy,
                    0.9,
                    format!("KDJ 金叉 J 从 {:.1} 反弹 (K={:.1} D={:.1})", p, k, d),
                );
            }
            if p > 100.0 && j <= 100.0 {
                return make_signal(
                    ...
                    Side::Sell,
                    0.9,
                    format!("KDJ 死叉 J 从 {:.1} 回落 (K={:.1} D={:.1})", p, k, d),
                );
            }
        }
...
impl IchimokuStrategy {
    ...
    fn min_history(&self) -> usize {
        self.senkou_b_p + self.displacement
    }
}
impl Strategy for IchimokuStrategy {
    ...
    fn on_bar(&mut self, bar: &Bar) -> Signal {
        ...
        let n = self.history.bars.len();
        if let (Some(sa), Some(sb), Some(t), Some(k)) = (
            ich.senkou_a[n - 1],
            ich.senkou_b[n - 1],
            ich.tenkan[n - 1],
            ich.kijun[n - 1],
...
impl Strategy for VortexStrategy {
    ...
    fn on_bar(&mut self, bar: &Bar) -> Signal {
        ...
        let n = self.history.bars.len();
        let plus = match v.plus[n - 1] {
            Some(v) => v,
            None => return hold(bar, "Vortex 未就绪"),
        };
        let minus = match v.minus[n - 1] {
            Some(v) => v,
            None => return hold(bar, "Vortex 未就绪"),
        };
        let diff = plus - minus;
        ...
        if let Some(p) = prev {
            if p <= 0.0 && diff > 0.0 {
                return make_signal(
                    ...
                    Side::Buy,
                    0.8,
                    format!("VI+({:.3}) 上穿 VI-({:.3})", plus, minus),
                );
            }
            if p >= 0.0 && diff < 0.0 {
                return make_signal(
                    ...
                    Side::Sell,
                    0.8,
                    format!("VI+({:.3}) 下穿 VI-({:.3})", plus, minus),
                );
            }
        }
        hold(bar, "Vortex 无交叉")
    ...
    fn on_bar(&mut self, bar: &Bar) -> Signal {
        ...
        if bull > 0.0 && bear < 0.0 {
            return make_signal(
                ...
                Side::Buy,
                0.6,
                format!("Elder 多头 bull={:.2} bear={:.2}", bull, bear),
            );
        }
        if bull < 0.0 && bear < 0.0 {
            return make_signal(
                ...
                Side::Sell,
                0.6,
                format!("Elder 空头 bull={:.2} bear={:.2}", bull, bear),
            );
        }
        hold(bar, "Elder 力量均衡")
    }
...
pub fn create_strategy(kind: StrategyKind) -> Box<dyn Strategy> {
    match kind {
        ...
        StrategyKind::Bollinger { period, num_std } => {
            Box::new(BollingerBandsStrategy::new(period, num_std))
        }
        StrategyKind::Supertrend { period, multiplier } => {
            Box::new(SupertrendStrategy::new(period, multiplier))
        }
