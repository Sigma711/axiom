...
pub fn delta(spot: f64, strike: f64, time: f64, rate: f64, volatility: f64, is_call: bool) -> f64 {
    if time <= 0.0 || volatility <= 0.0 {
        return 0.0;
    }
    let d1 = ((spot / strike).ln() + (rate + volatility * volatility / 2.0) * time)
...
/// Put: 同 + r × K × e^(-rt) × N(-d2)
pub fn theta(spot: f64, strike: f64, time: f64, rate: f64, volatility: f64, is_call: bool) -> f64 {
    if time <= 0.0 || volatility <= 0.0 {
        return 0.0;
    }
    let d1 = ((spot / strike).ln() + (rate + volatility * volatility / 2.0) * time)
    ...
    if is_call {
        (common - rate * strike * (-rate * time).exp() * norm_cdf(d2)) / 365.0
    } else {
        (common + rate * strike * (-rate * time).exp() * norm_cdf(-d2)) / 365.0
    }
}
...
pub fn rho(spot: f64, strike: f64, time: f64, rate: f64, volatility: f64, is_call: bool) -> f64 {
    if time <= 0.0 {
        return 0.0;
    }
    let d1 = ((spot / strike).ln() + (rate + volatility * volatility / 2.0) * time)
