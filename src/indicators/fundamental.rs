...
impl CompanyFinancials {
    ...
    /// PDF 第七章 2 节
    pub fn pe(&self) -> f64 {
        if self.eps == 0.0 { return 0.0; }
        self.price / self.eps
    }
    /// 市净率 PB = 股价 / 每股净资产
    /// PDF 第七章 3 节
    pub fn pb(&self) -> f64 {
        if self.book_value_per_share == 0.0 { return 0.0; }
        self.price / self.book_value_per_share
    }
    /// 市销率 PS = 股价 / 每股营收 (或 总市值 / 总营收)
    /// PDF 第七章 5 节
    pub fn ps(&self) -> f64 {
        if self.revenue_per_share == 0.0 { return 0.0; }
        self.price / self.revenue_per_share
    }
    /// PEG = PE / 盈利增长率(%)
    /// PDF 第七章 7 节
    pub fn peg(&self) -> f64 {
        if self.growth_rate == 0.0 { return 0.0; }
        self.pe() / (self.growth_rate * 100.0)
    }
    /// EV/EBITDA 倍数
    /// PDF 第七章 9 节
    pub fn ev_ebitda_ratio(&self) -> f64 { self.ev_ebitda }
    /// 股息率 = 每股股息 / 股价
    /// PDF 第七章 12 节
...
pub fn altman_z(working_capital: f64, retained_earnings: f64, ebit: f64,
               market_value: f64, total_liabilities: f64, sales: f64,
               total_assets: f64) -> f64 {
    ...
    let mv_ratio = if total_liabilities == 0.0 { 0.0 } else { market_value / total_liabilities };
...
pub fn roe(net_income: f64, avg_shareholders_equity: f64) -> f64 {
    if avg_shareholders_equity == 0.0 { return 0.0; }
...
pub fn roa(net_income: f64, avg_total_assets: f64) -> f64 {
    if avg_total_assets == 0.0 { return 0.0; }
