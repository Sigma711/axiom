...
pub fn free_float_ratio(free_float_shares: f64, total_shares: f64) -> f64 {
    if total_shares == 0.0 { return 0.0; }
...
pub fn short_interest_ratio(short_shares: f64, float_shares: f64) -> f64 {
    if float_shares == 0.0 { return 0.0; }
...
pub fn days_to_cover(short_shares: f64, avg_daily_volume: f64) -> f64 {
    if avg_daily_volume == 0.0 { return 0.0; }
...
pub fn piotroski_f_score(input: FScoreInput) -> i32 {
    let mut score = 0;
    if input.positive_roa { score += 1; }
    if input.positive_operating_cf { score += 1; }
    if input.roa_increasing { score += 1; }
    if input.accruals_decreasing { score += 1; }
    if input.current_ratio_improved { score += 1; }
    if input.shares_issued { score += 1; }
    if input.gross_margin_increased { score += 1; }
    if input.asset_turnover_increased { score += 1; }
    score
}
