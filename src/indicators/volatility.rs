...
pub fn historical_volatility(
    ...
    for (i, value) in out.iter_mut().enumerate().skip(period) {
        ...
        *value = Some(var.sqrt() * periods_per_year.sqrt());
    }
    out
}
...
pub fn mass_index(bars: &[Bar], ema_period: usize, sum_period: usize) -> Vec<Option<f64>> {
    ...
    let mut out = vec![None; n];
    for (i, value) in out.iter_mut().enumerate().skip(sum_period) {
        if i < double_ema.len() {
...
mod tests {
    ...
    fn test_bollinger_bands_contains_price() {
        ...
        let mut count = 0;
        for (i, price) in prices.iter().enumerate().take(100).skip(20) {
