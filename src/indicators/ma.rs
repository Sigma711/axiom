...
pub fn hma(prices: &[f64], period: usize) -> Vec<Option<f64>> {
    ...
    let diff: Vec<_> = half
        .iter()
        .zip(full.iter())
        .map(|(h, f)| Some(2.0 * (*h)? - (*f)?))
        .collect();
    optional_smooth(&diff, (period as f64).sqrt() as usize, wma)
}
/// Apply a smoother only to contiguous defined values. Missing warmup is not zero.
pub(crate) fn optional_smooth(
    values: &[Option<f64>],
    period: usize,
    smoother: fn(&[f64], usize) -> Vec<Option<f64>>,
) -> Vec<Option<f64>> {
    ...
    while start < values.len() {
        ...
        let mut end = start;
        while end < values.len() && values[end].is_some() {
            end += 1;
        }
        let data: Vec<_> = values[start..end].iter().filter_map(|x| *x).collect();
        out[start..end].copy_from_slice(&smoother(&data, period));
        start = end;
    }
    out
}
...
pub fn dema(prices: &[f64], period: usize) -> Vec<Option<f64>> {
    ...
    let e2 = optional_smooth(&e1, period, ema);
    e1.iter()
        .zip(e2.iter())
        .map(|(a, b)| Some(2.0 * (*a)? - (*b)?))
        .collect()
}
...
pub fn tema(prices: &[f64], period: usize) -> Vec<Option<f64>> {
    let e1 = ema(prices, period);
    let e2 = optional_smooth(&e1, period, ema);
    let e3 = optional_smooth(&e2, period, ema);
    e1.iter()
        .zip(e2.iter())
        .zip(e3.iter())
        .map(|((a, b), c)| Some(3.0 * (*a)? - 3.0 * (*b)? + (*c)?))
        .collect()
}
...
/// Williams Alligator: 三条 SMMA (蓝/红/绿) 鳄鱼指标
/// 蓝唇 (Jaw) = SMMA(13, 中价, 向前 8)
/// 红齿 (Teeth) = SMMA(8, 中价, 向前 5)
/// 绿唇 (Lips) = SMMA(5, 中价, 向前 3)
/// 中价 = (H+L)/2
pub fn alligator(bars: &[Bar]) -> AlligatorOutput {
    let mid: Vec<f64> = bars.iter().map(|b| (b.high + b.low) / 2.0).collect();
    AlligatorOutput {
        jaw: smma_offset(&mid, 13, 8),
        teeth: smma_offset(&mid, 8, 5),
        lips: smma_offset(&mid, 5, 3),
    }
}

pub struct AlligatorOutput {
    pub jaw: Vec<Option<f64>>,
    pub teeth: Vec<Option<f64>>,
    pub lips: Vec<Option<f64>>,
}

/// 平滑移动平均 (Wilder) — 与 rma 等价, 但带向前偏移 (用于鳄鱼等指标)
fn smma_offset(vals: &[f64], period: usize, offset: usize) -> Vec<Option<f64>> {
    ...
    for i in period..n {
        prev = (prev * (period as f64 - 1.0) + vals[i]) / period as f64;
        if i + offset < n {
            out[i + offset] = Some(prev);
        }
    }
    out
}

/// Williams Fractal: 5 根 K 线中, 中间那根是最高点 (上分型) 或最低点 (下分型)
/// 返回每个位置的分型方向: 1 = 上分型, -1 = 下分型, 0 = 无
