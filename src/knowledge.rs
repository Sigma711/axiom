...
fn market_microstructure() -> Vec<KnowledgeEntry> {
    vec![
        ...
        KnowledgeEntry {
            ...
            meaning: "内盘通常把靠近买一价成交的量归为主动卖出，外盘把靠近卖一价成交的量归为主动买入；实际分类由行情平台决定。".into(),
            signals: "主动买量高于主动卖量，说明该小时按交易所分类的买方更积极；不能单凭它预测下一根 K 线。".into(),
            pitfalls: "Binance 的主动买卖分类不等同于 A 股行情软件的内外盘；它也不是资金净流入。".into(),
            implementation: "Binance 已收盘现货 K 线字段 5、7、9、10；按成交资产单位分别比较。".into(),
            diagram: None,
        },
    ]
}
...
fn breadth_and_stats() -> Vec<KnowledgeEntry> {
    vec![
        ...
        KnowledgeEntry {
            ...
            meaning: "这里把当前策略每一期的净值收益，和同一时间戳的所选标的收盘收益放进一个滚动窗口，观察策略是否随市场同涨同跌。".into(),
            signals: "接近 1 表示近期同向，接近 -1 表示近期反向；先看窗口是否稳定、样本是否足够，再解释策略的市场暴露。".into(),
            pitfalls: "相关不等于因果，也会随窗口变化；策略与标的相关，不能证明两个资产适合配对交易。".into(),
...
fn more_volatility_volume() -> Vec<KnowledgeEntry> {
    vec![
        ...
        KnowledgeEntry {
            ...
            meaning: "把每个时段的主动买量减主动卖量后依次累加，用来观察成交方向的变化。".into(),
            signals: "价格与窗口内 CVD 同向时可相互佐证；背离只提示进一步检查，不自动判定真假突破。".into(),
            pitfalls: "本实践以 24 根已收盘 Binance 小时线的第一根为零起点，不是全历史 CVD，也不是资金净流入。".into(),
            implementation: "从 Binance K 线字段 5/9 与字段 7/10 各自计算主动买卖差，按小时在固定窗口内累计。".into(),
            diagram: None,
        },
        KnowledgeEntry {
            ...
            code_ref: "src/book.rs::market_recent_trade_summary".into(),

            category: "成交量分布".into(),
            name: "Volume Profile 成交量分布".into(),
            formula: "横轴 = 价格, 纵轴 = 该价位的成交量".into(),
            meaning: "揭示价格在哪里成交最多。".into(),
            signals: "POC与价值区描述本次近期成交集中位置，不保证未来支撑或阻力。".into(),
            pitfalls: "窗口仅为最近观测成交，不是完整交易时段；POC同量取高价，70%价值区从POC按相邻量扩展，同量向上。".into(),
            implementation: "src/book.rs::market_recent_trade_summary；按Binance真实逐笔成交价汇总base数量；PDF第十九章1节".into(),
            diagram: None,
        },
        KnowledgeEntry {
