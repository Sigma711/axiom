...
fn test_diagram_present_for_key_concepts() {
    ...
    let missing: Vec<String> = need_diagram
        .iter()
        .filter(|id| {
            !entries
                .iter()
...
fn test_candle_pattern_functions_exist() {
    ...
    // curr 范围在 prev 内
    assert_eq!(
        detect_inside_outside(&prev2, &inside),
...
fn test_indicator_functions_exist() {
    ...
    use axiom::indicators::momentum::{
        bop, coppock, demarker, elder_ray, fisher_transform, kst, rvi,
    };
    ...
    assert_eq!(t.len(), 3);
    assert_eq!(t, vec![70, -10, 60]); // 每个时点的横截面差，不跨时点累加
