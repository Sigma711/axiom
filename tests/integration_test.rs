...
use axiom::broker::{Broker, BrokerConfig, SimulatedBroker, new_order};
use axiom::data::{DataFeed, SyntheticFeed};
use axiom::engine::{BacktestEngine, EngineConfig, compare_strategies};
...
fn test_broker_market_order_with_commission_and_slippage() {
    ...
    let order = new_order(
        "BTCUSDT", Side::Buy, 0.5, Utc::now(),
        axiom::types::OrderType::Market, None,
    );
...
fn test_broker_rejects_oversold() {
    let mut broker = SimulatedBroker::new(
        BrokerConfig::default(), 50.0,
    );
    broker.set_market_price("BTCUSDT", 100.0);
    let order = new_order(
        "BTCUSDT", Side::Buy, 1.0, Utc::now(),
        axiom::types::OrderType::Market, None,
    );
    let fill = broker.place_order(order);
...
fn test_sma_cross_strategy_emits_signals() {
    ...
    for bar in &bars {
        ...
        if sig.side == Side::Buy { buy_signals += 1; }
        if sig.side == Side::Sell { sell_signals += 1; }
    }
...
fn test_backtest_engine_end_to_end() {
    let bars = SyntheticFeed::new(42).fetch_historical(
        "BTCUSDT",
...
fn test_compare_strategies_picks_winner() {
    let bars = SyntheticFeed::new(7).fetch_historical(
        "BTCUSDT",
        Utc.with_ymd_and_hms(2024, 1, 1, 0, 0, 0).unwrap(),
        300,
    ).unwrap();
    let strategies = vec![
        ("bh".to_string(), Box::new(BuyAndHoldStrategy::new()) as Box<dyn axiom::strategy::Strategy>),
        ("sma".to_string(), Box::new(SmaCrossStrategy::new(5, 20))),
        ("rsi".to_string(), Box::new(RsiStrategy::new(14, 70.0, 30.0))),
        ("random".to_string(), Box::new(RandomStrategy::new(99, 0.05, 0.05))),
    ];
    let out = compare_strategies(strategies, &bars, EngineConfig::default(), RiskConfig::default());
...
fn test_all_new_strategies() {
    let bars = SyntheticFeed::new(99).fetch_historical(
        "BTCUSDT",
        Utc.with_ymd_and_hms(2024, 1, 1, 0, 0, 0).unwrap(),
        200,
    ).unwrap();
